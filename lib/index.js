...
const BOM = '\uFEFF'
const GET_SCOPE_RULE_NAME = '__eslint-plugin-php-templates-get-scope'
const DECLARE_VARIABLES_RULE_NAME = '__eslint-plugin-php-templates-declare-variables'
const LINTER_ISPATCHED_PROPERTY_NAME = '__eslint-plugin-php-templates-verify-function-is-patched'
...
function getModuleFromCache(key) {
  ...
  if (typeof Linter === 'function' && typeof Linter.prototype.verify === 'function') {
    return Linter
  }
}
...
function iterateESLintModules(fn) {
  ...
  if (!found) {
    ...
    throw new Error(
      `eslint-plugin-php-templates error: It seems that eslint is not loaded.
If you think this is a bug, please file a report at https://github.com/tengattack/eslint-plugin-php-templates/issues
...
function patch(Linter) {
  ...
  Linter.prototype[verifyMethodName] = function (
    ...
    let messages
    const filename =
      typeof filenameOrOptions === 'object' ? filenameOrOptions.filename : filenameOrOptions
    const extension = path.extname(filename || '')

    // Hardcoded: only process .php files
    ...
    if (isPHP) {
      ...
      const pushMessages = (localMessages, code) => {
        messages.push.apply(
          messages,
          processor.postprocess([localMessages], code)
        )
      }

      const currentCodes = processor.preprocess(textOrSourceCode, filename)
